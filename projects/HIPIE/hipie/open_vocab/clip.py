...
class ClipAdapter(nn.Module):
    def __init__(self, name="ViT-B-32", normalize=True,pretrained = "openai"):
...
class MaskCLIP(ClipAdapter):
    ...
    @torch.no_grad()
    def build_text_embed(self, labels, verbose=False,always_cache=False):
        if str(labels) in self.cache_text:
            return self.cache_text[str(labels)]
        else:
            r = build_clip_text_embed(
                clip_model_name=self.clip,
                labels=labels,
                verbose=verbose,
            )
            if len(labels) > 1 or always_cache: # do not cache referring 
                self.cache_text[str(labels)] = r.cpu()
            return r
