...
class HungarianMatcherVL(nn.Module):
    ...
    @torch.no_grad()
    def compute_cost_label_vl(self, batch_idx, out_prob, targets, nf=1):
        ...
        cost = cost_class  #[num_query,num_gt]
        return cost
    
    @torch.no_grad()
    def memory_efficient_forward(self, outputs, targets, cost=["cls", "box", "mask"],force_box_loss=False):
        ...
        for b in range(bs):
            ...
            #pos_cost_class = alpha * ((1 - out_prob) ** gamma) * (-(out_prob + 1e-8).log())
            #cost_class = pos_cost_class[:, tgt_ids] - neg_cost_class[:, tgt_ids]
            cost_class = self.compute_cost_label_vl(
                b, out_prob, targets
            )
            ...
            if 'pred_masks' in outputs:
                out_mask = outputs["pred_masks"][b][0,:,0]
            ...
            # Final cost matrix
            if self.panoptic_box_loss and not force_box_loss:
    ...
    def forward(self, outputs, targets,mask_on=False,force_box_loss=False):
        """ Performs the matching
        ...
        """
        # overwrite
        if not force_box_loss:
            return self.memory_efficient_forward(outputs, targets,force_box_loss=force_box_loss)
        # ENC 
        with torch.no_grad():
            bs, num_queries = outputs["pred_logits"].shape[:2]
            ...
            # FILTERS
            # isthing = torch.cat([t['is_thing'] for t in targets], dim=0) # (N_obj, )
            # #is_thing_mask = target_is_thing.().reshape(-1,1)
            # cost_bbox[:, ~isthing] = cost_bbox[:, isthing].mean()
            # cost_giou[:, ~isthing] = cost_giou[:, isthing].mean()
            # cost_bbox[cost_bbox.isnan()] = 0.0
            # cost_giou[cost_giou.isnan()] = 0.0
            ...
            if mask_on:
                out_mask = torch.cat([x for x in outputs["pred_masks"]])  # [num_queries, H_pred, W_pred]
                #breakpoint()
                # gt masks are already padded when preparing target
                tgt_bbox = torch.cat([v["masks"] for v in targets])

                out_mask = out_mask[:, None]
                tgt_mask = tgt_mask[:, None]
                # all masks share the same set of points for efficient matching!
                point_coords = torch.rand(1, 112 * 112, 2, device=out_mask.device)
                # get gt labels
                tgt_mask = point_sample(
                    tgt_mask,
                    point_coords.repeat(tgt_mask.shape[0], 1, 1),
