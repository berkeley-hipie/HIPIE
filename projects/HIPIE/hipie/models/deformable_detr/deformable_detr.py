...
import random
from .jit_loss import sigmoid_ce_loss_jit,dice_loss_jit
...
class SetCriterion(nn.Module):
    ...
    """
    def __init__(self, matcher, weight_dict, losses, focal_alpha=0.25, mask_out_stride=4, ota=False, still_cls_for_encoder=False, cfg=None,matcher_bg=None,point_sample=True,
                 panoptic_box_loss=True):
        ...
        self.panoptic_box_loss = panoptic_box_loss
        # boxinst configs
        if cfg is not None:
            ...
            self.point_sample =  cfg.MODEL.POINT_SAMPLE
    ...
    def loss_boxes(self, outputs, targets, indices, num_boxes):
        ...
        target_is_thing = torch.cat([t['is_thing'][i] for t, (_, i) in zip(targets, indices)], dim=0)

        
        
        is_thing_mask = target_is_thing.float().reshape(-1,1)
        if self.panoptic_box_loss:
            pass
        else:
            is_thing_mask = torch.ones_like(is_thing_mask).float().to(is_thing_mask.device)
        adj_factor_is_thing = 1.0 / (is_thing_mask.sum() + 1e-6) * is_thing_mask.shape[0]
        if len(target_boxes) == 0 or is_thing_mask.sum() == 0:
            losses = {}
        ...
        if 'pred_boxious' in outputs:
            ...
            loss_boxiou = F.binary_cross_entropy_with_logits(src_iou_scores, tgt_iou_scores, reduction='none') * is_thing_mask.view(-1) 
            loss_boxiou = loss_boxiou.mean() * adj_factor_is_thing
        ...
        loss_bbox = loss_bbox * is_thing_mask * adj_factor_is_thing
        losses['loss_bbox'] = loss_bbox.sum() / num_boxes 
        loss_giou = giou_loss(box_ops.box_cxcywh_to_xyxy(src_boxes),box_ops.box_cxcywh_to_xyxy(target_boxes))
        loss_giou = loss_giou * is_thing_mask.view(-1) * adj_factor_is_thing
    ...
    def loss_masks(self, outputs, targets, indices, num_boxes):
        ...
        if self.point_sample:
            ...
            losses = {
                "loss_mask": sigmoid_focal_loss_jit(point_logits, point_labels, num_boxes),
                "loss_dice": dice_loss_jit(point_logits, point_labels, num_boxes),
            }
        else:
    ...
    def forward(self, outputs, targets, indices_list):
        ...
        if 'enc_outputs' in outputs:
            ...
            indices = self.matcher.forward(enc_outputs, bin_targets,force_box_loss=True)
            for loss in self.losses:
