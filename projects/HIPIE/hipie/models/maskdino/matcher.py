...
class HungarianMatcher(nn.Module):
    ...
    def __init__(self, cost_class: float = 1, cost_mask: float = 1, cost_dice: float = 1, num_points: int = 0,
                 cost_box: float = 0, cost_giou: float = 0, panoptic_on: bool = False,vl_loss=False):
        """Creates the matcher
    ...
    def memory_efficient_forward(self, outputs, targets, cost=["cls", "box", "mask"]):
        ...
        for b in range(bs):
            ...
            if self.vl_loss:
                ...
                cost_class = self.compute_cost_label(
                    b, out_prob, targets
                )
    ...
    @torch.no_grad()
    def compute_cost_label(self, batch_idx, out_prob, targets, nf=1):
        tgt_ids = targets[batch_idx]["labels"]
        out_prob =  out_prob[batch_idx] 
        alpha = 0.25
        gamma = 2.0
        neg_cost_class = (1 - alpha) * (out_prob ** gamma) * (-(1 - out_prob + 1e-8).log())
        pos_cost_class = alpha * ((1 - out_prob) ** gamma) * (-(out_prob + 1e-8).log())
