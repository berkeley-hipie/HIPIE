...
class MaskDINODecoder(nn.Module):
    ...
    def __init__(
        ...
        self.dynamic_label_enc = dynamic_label_enc
        self.dynamic_label_enc_dropout = dynamic_label_enc_dropout
        ...
        if self.dynamic_label_enc:
            self.resizer = FeatureResizer(
                    input_feat_size=768,
                    output_feat_size=hidden_dim, # 256
                    dropout=self.dynamic_label_enc_dropout
                )
        else:
    ...
    def from_config(cls, cfg, in_channels, mask_classification):
        ...
        ret['dynamic_label_enc'] = cfg.MODEL.MaskDINO.DYNAMIC_LABEL_ENC
        ret['dynamic_label_enc_dropout'] = cfg.MODEL.MaskDINO.DYNAMIC_LABEL_ENC_DROPOUT
        return ret

    def prepare_for_dn(self, targets, tgt, refpoint_emb, batch_size,lang_feat_pool=None):
        """
        ...
        if self.training:
            ...
            known_bid_pt = batch_idx.repeat(scalar, 1).view(-1)
            # noise
            ...
            if self.dynamic_label_enc:
                label_enc = self.resizer(lang_feat_pool)
    ...
    def forward(self, x, mask_features, masks, targets=None,lang_feat_pool=None):
        """
        ...
        if self.dn != "no" and self.training:
            ...
            input_query_label, input_query_bbox, tgt_mask, mask_dict = \
                self.prepare_for_dn(targets, None, None, x[0].shape[0],lang_feat_pool)
