...
from ..deformable_detr.deformable_detr import (dice_loss, sigmoid_focal_loss, token_sigmoid_binary_focal_loss)
...
token_sigmoid_binary_focal_loss_jit = torch.jit.script(
...
class SetCriterion(nn.Module):
    ...
    def __init__(self, num_classes, matcher, weight_dict, eos_coef, losses,vl_loss,
    ...
    def loss_labelsVL(self, outputs, targets, indices, num_boxes, log=False):
        ...
        num_boxes = num_boxes # len(idx[0]) # if self.ota else num_boxes
        if num_boxes == 0:
            ...
            target_classes_onehot = target_classes_onehot.detach().to(src_logits.device)
            text_mask = outputs['text_masks'].detach().to(src_logits.device)
    ...
    def loss_boxes_panoptic(self, outputs, targets, indices, num_boxes):
        ...
        isthing = torch.cat([t['is_thing'][i] for t, (_, i) in zip(targets, indices)], dim=0)
    ...
    def get_loss(self, loss, outputs, targets, indices, num_masks):
        loss_map = {
            'labels': self.loss_labelsVL if self.vl_loss else self.loss_labels,
